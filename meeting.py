...
if 'direct_input' not in st.session_state:
    st.session_state.direct_input = ""  # 초기값 설정
if 'combined_text' not in st.session_state:
    st.session_state.combined_text = ""
if 'summary' not in st.session_state:
    st.session_state.summary = ""

# 음성 파일 AssemblyAI STT 처리
...
# 직접 입력 - 수정된 부분 (위젯이 세션 상태를 직접 관리)
st.subheader("직접 입력")
st.text_area(
    "회의 내용을 직접 입력하세요",
    value=st.session_state.direct_input,  # 세션 상태 값 사용
    height=200,
    key="direct_input"  # 키 지정으로 자동 관리
)

# 통합 및 요약 생성
if st.button("회의록 통합 및 요약 생성", type="primary", use_container_width=True):
    # 입력 수집 (세션 상태에서 직접 가져옴)
    inputs = []
    if st.session_state.transcript:
        inputs.append(st.session_state.transcript)
    if st.session_state.text_content:
        inputs.append(st.session_state.text_content)
    if st.session_state.direct_input:  # 세션 상태 값 사용
